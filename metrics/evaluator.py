"""
Модуль evaluator.py - расчёт метрик качества для моделей обнаружения фрода.

Содержит функции:
- compute_metrics() - расчёт Accuracy, Precision, Recall, F1, ROC-AUC, Business Cost
- format_metrics_table() - форматирование таблицы с метриками
- evaluate_with_stress() - оценка модели на стресс-сценариях
- evaluate_all_models() - оценка всех моделей
- calculate_drawdown() - расчёт просадки метрик в процентах
- format_results_with_drawdown() - форматирование таблицы с просадкой
"""
...
def evaluate_with_stress(model, X, y_true, scenario_names, cost_fp=1, cost_fn=10):
    """Оценивает модель на стресс-сценариях"""
    ...
    for scenario in scenario_names:
        ...
        results[scenario] = compute_metrics(
            y_stressed, y_pred, y_proba, cost_fp, cost_fn
        )
...
def evaluate_all_models(models_dict, X, y_true, scenario_names, cost_fp=1, cost_fn=10):
    ...
    for model_name, model in models_dict.items():
        all_results[model_name] = evaluate_with_stress(
            model, X, y_true, scenario_names, cost_fp, cost_fn
        )
    return all_results
    ...
    for metric in normal_metrics:
        if metric in stress_metrics and normal_metrics[metric] != 0:
            ...
            if not metric.endswith("_count") and metric != "Business_Cost":
                drawdown[metric] = round(
                    (
                        (stress_metrics[metric] - normal_metrics[metric])
