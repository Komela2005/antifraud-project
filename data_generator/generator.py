...
def generate_fraud_dataset(
    ...
    for client_id in range(1, n_clients + 1):
        ...
        city_pop = np.random.choice(
            [5000, 50000, 500000, 5000000], p=[0.2, 0.3, 0.3, 0.2]
        )
        ...
        if np.random.rand() < 0.05:
            phone_last_change = start_date + timedelta(
    ...
    for _ in range(n_legit):
        ...
        ts = start_date + timedelta(
            seconds=np.random.randint(0, delta_days * SECONDS_PER_DAY)
        )
    ...
    df["time_deviation_hour"] = cyclic_deviation(
        df["transaction_hour"].values, df["typical_hour_client"].values
    )
    ...
    def get_category(is_fraud):
        if is_fraud == 0:
            return np.random.choice(
                categories, p=[0.2, 0.15, 0.1, 0.2, 0.15, 0.1, 0.09, 0.01]
            )
        else:
            return np.random.choice(
                categories, p=[0.15, 0.1, 0.2, 0.3, 0.1, 0.05, 0.05, 0.05]
            )
    ...
    shape = df.loc[legit_mask, "avg_amount_client"] ** 2 / (
        df.loc[legit_mask, "std_amount_client"] ** 2 + 1e-6
    )
    scale = (df.loc[legit_mask, "std_amount_client"] ** 2 + 1e-6) / (
        df.loc[legit_mask, "avg_amount_client"] + 1e-6
    )
    df.loc[legit_mask, "amount"] = np.random.gamma(shape, scale)

    fraud_avg = df.loc[fraud_mask, "avg_amount_client"].values
    factors = np.where(
        np.random.rand(len(fraud_avg)) < 0.5,
    ...
    df.loc[mask_low, "is_contactless"] = np.random.binomial(
        1, 0.25, size=mask_low.sum()
    )
    mask_low_fraud = (~df["category"].isin(high_cats)) & (df["is_fraud"] == 1)
    df.loc[mask_low_fraud, "is_contactless"] = np.random.binomial(
        1, 0.35, size=mask_low_fraud.sum()
    )

    contactless = df["is_contactless"] == 1
    df["nfc_duration_ms"] = 0.0
    df.loc[contactless & (df["is_fraud"] == 0), "nfc_duration_ms"] = np.random.normal(
        450, 150, size=(contactless & (df["is_fraud"] == 0)).sum()
    )
    df.loc[contactless & (df["is_fraud"] == 1), "nfc_duration_ms"] = np.random.normal(
        550, 200, size=(contactless & (df["is_fraud"] == 1)).sum()
    )
    ...
    mask_other = (df["is_fraud"] == 0) & (
        ~df["category"].isin(["travel", "online_shopping"])
    )
    ...
    df["is_unusual_amount"] = (
        (amount > avg + 2.5 * std) | (amount < avg - 2.5 * std)
    ).astype(int)
    ...
    def phone_changed(row):
        if pd.isna(row["phone_last_change"]):
            return 0
        diff_hours = (
            row["trans_date"] - row["phone_last_change"]
        ).total_seconds() / 3600
    ...
    df["device_age_days"] = np.random.exponential(365, len(df)) * (
        1 + 0.3 * df["is_fraud"] + np.random.normal(0, 0.2, len(df))
    )
...
def get_features_for_iforest() -> list:
    ...
    categories = [
        ...
        "category_travel",
        "category_online_shopping",
        "category_restaurant",
        "category_transport",
        "category_other",
        "category_cash_deposit",
    ]
...
def generate_fraud_subset(
    ...
    if not 100 <= subset_size <= 2000:
        raise ValueError(
            f"subset_size должен быть от 100 до 2000, получено {subset_size}"
        )

    if subset_size > full_size:
        raise ValueError(
            f"subset_size ({subset_size}) не может быть больше full_size ({full_size})"
        )
    ...
    if use_stratification:
        ...
        subset_df = subset_df.sample(frac=1, random_state=random_state).reset_index(
            drop=True
        )
    else:
