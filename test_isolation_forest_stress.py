...
for scenario in scenarios:
    ...
    y_true = df_stressed['is_fraud']
    
    # Предсказание Isolation Forest
    y_pred_raw = iso_forest.predict(X_test)
    # Isolation Forest возвращает 1 (норма) и -1 (аномалия)
...
for _, row in df_results.iterrows():
    ...
    if scenario != 'normal':
        drop_accuracy = (normal_metrics['accuracy'] - row['accuracy']) / normal_metrics['accuracy'] * 100
