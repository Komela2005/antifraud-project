...
NUMERIC_FEATURES_20 = [
    'amount', 'transaction_hour', 'day_of_week', 'distance_km',
    'nfc_time_exceeded', 'nfc_duration_ms', 'is_unusual_amount',
    'is_unusual_time', 'sms_anomaly_6h', 'phone_changed_48h',
    'device_risk_high', 'device_age_days', 'suspect_cash_deposit',
    'new_beneficiary_after_self_transfer', 'age', 'city_pop',
    'avg_amount_client', 'std_amount_client', 'typical_hour_client',
    'device_risk'
]
...
def prepare_data_for_catboost(X):
    ...
    expected_order = [
        'amount', 'category', 'transaction_hour', 'day_of_week', 'distance_km',
        'nfc_time_exceeded', 'nfc_duration_ms', 'is_unusual_amount',
        'is_unusual_time', 'sms_anomaly_6h', 'phone_changed_48h',
        'device_risk_high', 'device_age_days', 'suspect_cash_deposit',
        'new_beneficiary_after_self_transfer', 'age', 'city_pop',
...
selected_models = st.sidebar.multiselect(
    "Models to compare", options=all_models, default=all_models,
...
compare_button = st.sidebar.button("Run analysis", type="primary", help="Start model evaluation")
...
if compare_button:
    ...
    try:
        ...
        st.success(f"Data loaded! {len(X_classic)} rows, {len(X_classic.columns)} features.")
...
if st.session_state.classic_df_full is not None:
    ...
    if (
        ...
        fig = px.bar(chart_df, x="Model", y=["F1 Classic", "F1 Stress"], barmode="group")
        ...
        roc_models = [m for m in selected_models if m in st.session_state.proba_dict and st.session_state.proba_dict[m] is not None]
        
        if roc_models:
            selected_roc_model = st.selectbox("Select model for ROC curve", roc_models, key="roc_select")
        ...
        if selected_models:
            selected_cm_model = st.selectbox("Select model for Confusion Matrix", selected_models, key="cm_select")
            cm_mode = st.radio("Mode", ["Classic", "Stress"], horizontal=True, key="cm_mode")
            
            all_models_dict = load_all_models()
            model = all_models_dict[selected_cm_model]
            
            if cm_mode == "Classic":
                X_processed = prepare_model_data(selected_cm_model, model, st.session_state.X_classic)
            else:
                X_processed = prepare_model_data(selected_cm_model, model, st.session_state.X_stress)
            ...
            y_true = st.session_state.y_classic_final if cm_mode == "Classic" else st.session_state.y_stress
        ...
        importance_models = [m for m in selected_models if m in ['Random Forest v1', 'Random Forest v2', 'CatBoost v1', 'CatBoost v2']]
        
        if importance_models:
            selected_imp_model = st.selectbox("Select model for feature importance", importance_models, key="imp_select")
            ...
            if selected_imp_model:
                ...
                fig_imp = plot_feature_importance(model, selected_imp_model, feature_names)
                if fig_imp:
